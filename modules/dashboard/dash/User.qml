...
Item {
    ...
    StyledText {
        ...
        text: qsTr("Thời gian hoạt động: ") + SysInfo.uptime.split(",").slice(0, 2).join(",").replace(/\bdays?\b/g, qsTr("ngày")).replace(/\bhours?\b/g, qsTr("giờ")).replace(/\bminutes?\b/g, qsTr("phút")) // Max 2 components
