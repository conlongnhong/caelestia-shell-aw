...
void BlobShape::updatePolish() {
    ...
    for (BlobShape* other : m_group->shapes()) {
        ...
        if (include) {
            ...
            const float c = dm(0, 1), matrixD = dm(1, 1);
            ...
            r.screenHalfY = std::abs(b) * r.hw + std::abs(matrixD) * r.hh;
